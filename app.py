...
def user_input_features():
    Administrative = st.sidebar.slider('Administrative', 0, 30, 0)
    Administrative_Duration = st.sidebar.slider('Administrative_Duration', 0.0, 3000.0, 0.0)
    Informational = st.sidebar.slider('Informational', 0, 30, 0)
    Informational_Duration = st.sidebar.slider('Informational_Duration', 0.0, 3000.0, 0.0)
    ProductRelated = st.sidebar.slider('ProductRelated', 0, 300, 0)
    ProductRelated_Duration = st.sidebar.slider('ProductRelated_Duration', 0.0, 30000.0, 0.0)
    BounceRates = st.sidebar.slider('BounceRates', 0.0, 0.2, 0.0)
    ExitRates = st.sidebar.slider('ExitRates', 0.0, 0.2, 0.0)
    PageValues = st.sidebar.slider('PageValues', 0.0, 400.0, 0.0)
    SpecialDay = st.sidebar.slider('SpecialDay', 0.0, 1.0, 0.0, 0.1)
